...
## Visual Design — Editorial, NOT GitHub (HARD RULE — DO NOT REVERT)

**Critical**: this product clones GitHub's *capabilities* (repos, PRs, issues, Actions, etc.) but ships with the **Editorial** design system — a calm, magazine-grade aesthetic with warm cream paper, ink black text, single rust accent, and Fraunces / Inter Tight / JetBrains Mono typography. **The Editorial design is the locked product theme.** Do NOT replace it with GitHub's Primer styling, do NOT introduce GitHub's color palette, do NOT swap Fraunces/Inter Tight/JetBrains Mono for system fonts, even if a previous iteration looked GitHub-like or a screenshot in `ralph/screenshots/inspect/` shows GitHub chrome.

**The reference for visual design is `design/project/Prototype.html` and `og-screens-*.jsx` — NOT the inspect screenshots of github.com.** Inspect screenshots show *information architecture and interactions* to clone; visuals must always come from the Editorial prototype.

### Banned values (NEVER write these in any file under `web/`)

| Banned | Why | Editorial equivalent |
| --- | --- | --- |
| `#0969da` (GitHub blue) | accent | `var(--accent)` (rust) — and only sparingly |
| `#1f883d` / `#1a7f37` (GitHub green) | success | `var(--ok)` for text, `.chip.ok` for chips |
| `#cf222e` / `#82071e` (GitHub red) | error | `var(--err)`, `.chip.err`, `--err-soft` for backgrounds |
| `#f6f8fa` (GitHub canvas-subtle) | surface | `var(--bg)` (`#faf7f2`) or `var(--surface-2)` |
| `#1f2328` (GitHub fg-default) | text | `var(--ink-1)` (`#14120e`) |
| `#d0d7de` (GitHub border-default) | border | `var(--line)` / `var(--line-strong)` |
| `#59636e` (GitHub fg-muted) | muted text | `var(--ink-3)` |
| `#f1aeb5` / `#fff1f3` (GitHub red border/bg) | error chrome | `var(--err)` / `var(--err-soft)` |
| `Octicon`, `@primer/*` imports | iconography/components | inline SVG following `og-icons.jsx`, design-system primitives |
| `font-family: Helvetica/Arial/system-ui` direct | type | `var(--sans)` / `var(--display)` / `var(--mono)` |

### MANDATORY pre-commit grep check (run before `git commit`)

```bash
# Fail loudly if any banned GitHub-style color/import slipped into web/.
if rg -nE '#(0969da|1f883d|1a7f37|cf222e|82071e|f6f8fa|1f2328|d0d7de|59636e|f1aeb5|fff1f3)\b|@primer/|Octicon' web/src/ --glob '!**/og*.css'; then
  echo "VISUAL REGRESSION: GitHub-style values present. Replace with Editorial tokens before commit."; exit 1
fi
```

Run this check before every `git commit`. If matches surface, **fix them in the same commit** by swapping to the Editorial token from the table above (or `.btn` / `.chip` / `.card` primitives). Do NOT bypass the check, do NOT add files to a `.gitignore` style allowlist, do NOT commit the regression.

### Before writing any UI code

1. Read `AGENTS.md` "Design System" section and `web/AGENTS.md` "Design System — Editorial" section.
2. Browse `design/project/Prototype.html`, `design/project/og.css`, and the relevant `og-screens-*.jsx` for the screen you're building (Landing/Login/Dashboard/Notifications → `og-screens-1.jsx`; Repo/File → `og-screens-2.jsx`; Pulls/PR → `og-screens-3.jsx`; Issues/Actions → `og-screens-4.jsx`; TopBar/Sidebar/⌘K → `og-shell.jsx`).
3. Use the live tokens at `web/src/app/og.css` (CSS variables: `--bg`, `--ink-1`, `--accent`, `--display`, `--sans`, `--mono`, `--radius`, etc.) and the pre-styled component primitives (`.btn`, `.chip`, `.card`, `.input`, `.av`, `.tabs`, `.list-row`, `.kbd`, `.palette-*`, `.t-display`, `.t-h1`-`.t-h3`, `.t-mono`, `.t-label`, `.t-num`).
4. Match the Editorial layout language: 1240px content max, generous whitespace, single rust accent, Fraunces only for display moments, mono only for code/IDs/kbd hints, semantic chips (`.chip.ok` / `.chip.warn` / `.chip.err`) for status.

When the PRD says "render the X tab like GitHub does", interpret that as **the same information architecture and interactions** GitHub has — but rendered with the Editorial design system. The chats in `design/chats/` show the design intent. Always prefer porting from `design/project/og-screens-*.jsx` over inventing fresh layout.

If you find a previous iteration committed GitHub-style colors in a file you're touching, fix them as part of this iteration's commit — do not leave the regression behind.
...
**Make targets to use** (already wired):
- `make check` → cargo check + clippy + (when web/ exists) tsc + biome
- `make test` → cargo test + (when web/ exists) vitest
- `make test-e2e` → playwright (only when web/ exists)
- `make build` → cargo build --release + next build
- `make dev` → runs Rust API on :3016 and Next.js on :3015 in parallel
